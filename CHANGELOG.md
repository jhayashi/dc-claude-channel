...
- **`scripts/export-chat.ts` exports one chat — text, speakers and uploaded files — into a portable folder.** Step one of the n-way memory migration ([n-way#6](https://github.com/jhayashi/n-way/issues/6)): step two reads the folder and seeds an agent's wiki from it. Writes `transcript.md` (chronological, message id in every heading so a model reading only the markdown can still cite `chat:<chatId>/msg:<msgId>`), `messages.jsonl` (one structured record per message), `attachments/` (originals, named `<msgId>-<original filename>`) and `manifest.json`. Both text outputs render from the same in-memory model, so they cannot disagree about a file. The source is the dc-core sqlite DB rather than the subagent's session JSONL or the event logs, because it is the only one holding *both* speakers and the uploaded bytes — and it keeps a file and its caption in the same row, which is exactly the association the import step needs. Opened read-only, so it is safe to run while the dispatcher is up: `accounts.lock` guards against a second dc-core process, not a reader. Select a chat by id or by a name substring that must match exactly one chat (an ambiguous substring lists the candidates and exits 1 rather than guessing); `--list` enumerates chats with visible-message counts. Hidden messages are excluded, which drops the emoji status pings the dispatcher posts mid-turn; dc-core system messages are kept but tagged `kind: "system"`, since some carry real signal ("Tap to review 📄 …") and the importer can weight down what it cannot recover. A blob missing from `dc.db-blobs` is a warning, not a failure — an export that aborts two-thirds through is worse than one that says what it lost.
