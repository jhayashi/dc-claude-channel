...
**Resume-from-serial (optional, for apps with long-lived per-chat instances):** `setUpdateListener(fn, 0)` is the simple default, but an app that accumulates a lot of history in one long-lived instance (one card reused across many sends, like the file reviewer) can instead cache its reconstructed state in `localStorage` — `{lastSerial, ...state}` — written after each processed update (debounce if updates arrive in bursts), and pass `lastSerial` as the second argument to `setUpdateListener` on a later boot instead of `0`. DC then only replays what's new since that serial. Treat the cache as pure optimization: a missing or corrupt cache at read/boot time must fall back to `setUpdateListener(fn, 0)` (full replay) with no other change in behavior. Quota-exceeded is a separate, write-time concern — it's handled by trimming the payload and retrying, or by skipping the write if even the trimmed write fails — and does not by itself reset the read-time resume point to 0 (a successful trimmed retry still advances `lastSerial`, so the next open resumes from there, not from 0). Never let cache failure become a user-visible break either way. `plugin/webxdc/file-reviewer.html`'s `frCache.v1` (#113) is the reference implementation, including a capped document count with one-time eviction (evict oldest, single toast per eviction batch, not per file).
