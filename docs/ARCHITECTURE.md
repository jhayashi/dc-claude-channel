...
- `plugin/apps/` — App implementations:
  - `file-reviewer-app.ts` — File reviewer: rendered markdown + syntax-highlighted source + inline commenting (1 tool, event-driven updates). Auto-detects Marp decks (YAML frontmatter `marp: true`, or a no-frontmatter doc that starts with `---\n` and splits into 2+ sections) and renders them as an interactive slide deck with slide-aware comment anchors. Detection in `plugin/marp-detect.ts`. `dc_send_slides` is a thin alias to `dc_send_file`. Kebab menu (v1.3.2+) consolidates two viewer-side actions: **Find in document** (cross-node flat-text + nodeMap match, debounced 100ms, capped at 1000 matches, n/N navigation, Esc clears) and **Export file** (sends the open doc back to the chat as a real DC attachment — 100 KB cap, three-tier filename rules where title-extension wins → language-mapped → `.txt` fallback; multi-chunk docs assemble into one attachment).
...
**Per-agent tool access (v1.4+):** The `tools:` frontmatter field is a single comma-separated list of built-in tool names (e.g. `Read`, `Edit`, `Bash`) and MCP tool references. CC's runtime permission system accepts `mcp__<server>` as a wildcard via the `--allowed-tools` CLI flag (which the dispatcher passes at spawn — see "Subagent model"), but CC's frontmatter `tools:` parser treats `mcp__<server>` as a literal tool name. To make agent .mds portable to terminal CC's Task delegation path (where the frontmatter is the only allowlist source), `agents.ts:saveAgent` expands the bare `mcp__dc` prefix into the full enumerated `mcp__dc__<tool>` list on every write. `mcp__dc` is mandatory because every DC tool call (reply, react, history, schedule, …) routes through that prefix; `assertCanSpawn` in `dispatcher/subagent-cache.ts` refuses to spawn an agent whose CSV lacks it. The agent-setup WebXDC card still surfaces a collapsible tool picker; under the hood the card's "built-ins" + "MCP servers" sub-pickers collapse to a single CSV on save and split back on edit.

**Known limitation — non-DC MCP servers and Task delegation.** The DC prefix expansion above only covers `mcp__dc__*`. Agents that reference other servers (e.g. `mcp__claude_ai_Gmail`, `mcp__claude_ai_Google_Calendar` — used by the shipped `email-digest`, `exec-assistant`, and similar templates) keep the bare prefix on disk because the per-server tool list is dynamic (it depends on the user's per-account OAuth grants and changes per CC release). At runtime this is fine: the dispatcher's `--allowed-tools` flag treats `mcp__claude_ai_Gmail` as a server wildcard, so the subagent can call any Gmail tool the OAuth scope permits. The breakage is on the terminal-CC Task delegation path — if a terminal session uses `Task → <agent-with-Gmail>`, CC's frontmatter parser won't expand the prefix and Gmail tools will be blocked. For now, agents intended for Task delegation should enumerate the specific tool names they need.
...
Subagents run under the agent .md's `permissionMode` (the dispatcher reads the value at spawn and forwards it as `--permission-mode <value>`, defaulting to CC's `default` when unset). For an untrusted agent, when Claude wants to run a built-in tool like Bash or Edit, a PreToolUse hook fires, connects to the dispatcher's Unix socket, and blocks waiting for a verdict. The dispatcher forwards the prompt to the existing permissions-app WebXDC flow in the bound chat and writes the user's Allow/Deny back to the hook. The hook matcher list only covers built-ins (`Bash`, `Edit`, `Write`, `NotebookEdit`, `WebFetch`, `WebSearch`); MCP tool calls don't trigger the hook, so the dispatcher additionally passes `--allowed-tools <agent.tools CSV>` at spawn to pre-grant every tool the agent's .md declares. Trusted agents (`permissionMode: bypassPermissions`) bypass both layers — toggling trust mid-session evicts the cached subagent so the new flag takes effect on the next message.
...
**Turn interruption (v1.3.2+, #21 + #45):** `/stop` slash command and edit-a-prior-message both funnel through `subagentCache.evictChat`. Three layers:

1. **Process-tree kill.** `SubagentProcess.killTree` walks the full descendant tree via a single `ps -e -o pid=,ppid=` snapshot captured *before* the SIGTERM (post-kill reparenting would erase the linkage), inverts it to a children-map in memory, BFS-collects descendants, then sends SIGTERM depth-first; 2s grace, then SIGKILL the same way. The tree walk is essential because claude's Bash tool `setsid`s its tool shells into their own process groups — `process.kill(-pid, signal)` on claude's pgrp would leave bash/curl/python grandchildren orphaned and reparented to init. Single ps snapshot rather than recursive `pgrep -P` to close a transient-PID race window. Windows fallback (taskkill /T /F) tracked as #93/#95. Verified empirically via `plugin/scripts/smoke-process-group-kill.sh`.

2. **In-flight readFrame cancellation.** `SubagentProcess.close()` flips `this.closed = true` and calls `abortPendingReaders` *before* the kill cascade. `abortPendingReaders` rejects the in-flight `readFrame` Promise synchronously via a `pendingReject` slot captured inside the executor — so the awaiting `send()` throws immediately instead of sitting out the multi-hour turn timeout. The `exit` handler also calls it (covers clean child exits). `readFrame` short-circuits with `Promise.reject` if called after `close()`.

3. **Server-side error filter.** The dispatch catch in `server.ts:runSubagentTurn` recognizes shutdown-class errors (regex `^subagent\b.*\b(closed|evicted|exited)\b`) and skips the chat-side "⚠️ Internal error" toast — the events are still recorded to `turns-*.log` with `exitReason: user_abort | lru_evict | crash` for diagnostics.
