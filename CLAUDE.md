...
**Import/export (v1.4.18+, #130):** export (manage card) writes `<agentId>.md` in the same frontmatter-markdown format `saveAgent` writes and `importAgentFromMarkdown` parses — `agents.exportAgentMarkdown(def)` is THE export serializer; never `YAML.stringify` an AgentDef for transport. Import intercepts `.yaml`/`.yml` and `.md` attachments (`dispatcher/agent-import.ts`); `.md` requires an agent-frontmatter sniff (`name` or `model` key) so ordinary markdown documents pass through to the subagent untouched. For the v1.2 wall+coach creation flow, per-agent tool-access restrictions, and migration details, see [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) and the spec at `docs/superpowers/specs/2026-05-16-cc-agent-compatibility-design.md`.
...
**Posting turn results (v1.4.18+, #128):** every dispatch path whose outcome the user must see posts through `dispatcher/turn-post.ts`'s `postTurnResult` (final text + policy-denial summary) — `runSubagentTurn`, the scheduler, edit-as-interrupt, the native-moment offers, file-reviewer comment turns, and the reaction router all share it. App code uses `ctx.dispatchAndPost`; `ctx.dispatchAndCollect` is ONLY for callers that consume the text themselves (familiar `requestLLM`, teleport's CHAT_NAME extraction). Adding a new dispatch path that awaits `subagentCache.dispatch` and drops the result is the #128 bug class — don't.
...
**Write side (v1.1.5+).** Records are populated on every successful pair (`completePairing` hook) and lazily backfilled on dispatcher startup for legacy installs (`backfillFromAllowlist`). Role assignments via the contacts card's role picker write through `setContactRole(agentId, contactId, role)` and audit-log a `RoleAssignmentEvent`. **v1.4.18+ (#133):** `dc_set_contact_role({chat_id, contact_id, role})` is the directly-callable equivalent ("give Alice full access" as a chat message) — the `dc_rebind_chat` pattern: `requiresCapability: 'infrastructure'`, no §6 callback (authenticated `fromId`), resolves the managed agent from the chat's binding. It's the sanctioned recovery in multi-human groups, where the card's tap always refuses.
...
- **`help`** (#108, v1.4.19) — opened by `/help` only (falls back to HELP_TEXT if the card send fails). Static searchable index of all 42 validated user journeys across 8 topics; content injected at build time from `help-content.ts` (Commands topic generated from `slash-commands.ts` — the parity suite covers table↔router↔text↔card); per-journey **Try it** drafts the phrase as the USER's own message via `webxdc.sendToChat` (never server-dispatched — execute lanes need the sender's authenticated fromId). Every phrase carries a `verify` tier (#138): t1 asserted in CI, t2 via the gated paid smoke (`DC_HELP_SMOKE=1`, see `test/integration/README.md`), t3 in `docs/HELP-SMOKE.md`.
